...
const MODEL_BYTES = 2_480_000;      // ориентир для процента, если нет Content-Length
...
/* модель сжата meshopt'ом — без декодера её не распаковать */
new GLTFLoader()
  ...
  gltf => { build(gltf); },
  ev => {
    /* Total бывает 0, если сервер отдаёт без Content-Length (gzip на лету) */
    const total = ev.total || MODEL_BYTES;
    const pct = clamp(ev.loaded / total, 0, 0.99);
    if (dom.loaderBar) dom.loaderBar.style.transform = `scaleX(${pct})`;
    if (dom.loadPct) dom.loadPct.textContent = Math.round(pct * 100) + '%';
  },
  err => { console.error('GLB load failed', err); fail(); }
);

/* Каталог не должен пропадать, если 3D не сложилось или посетитель
   пролистал вниз, не дожидаясь модели: переносим карточки в обычную секцию. */
let staticMounted = false;
function mountStaticCatalog() {
  if (staticMounted) return;
  staticMounted = true;
  const sec = document.createElement('section');
  sec.className = 'catalog-static';
  sec.id = 'catalog-static';
  const head = document.createElement('div');
  head.className = 'sect__head';
  head.innerHTML = '<span class="dots"><i></i><i></i></span>' +
                   '<h2 class="sect__title">КАТАЛОГ ТЕХНИКИ</h2>' +
                   '<p class="sect__sub">Четыре направления — от бюджетной офисной машины ' +
                   'до сборки под конкретную игру</p>';
  sec.appendChild(head);
  sec.appendChild(dom.layer);                 // забираем слой с карточками из сцены
  dom.stage.after(sec);
...
function fail() {
  ...
  document.body.classList.remove('is-loading-model');
  document.body.classList.add('is-loaded', 'is-no-3d');
  if (dom.hint) {
    dom.hint.innerHTML = location.protocol === 'file:'
      ? '<span class="micro">Откройте сайт через start.bat — из файла 3D не загружается</span>'
      : '<span class="dots"><i></i><i></i></span><span class="micro">Листайте — каталог ниже</span>';
  }
  mountStaticCatalog();
}
...
/* Первый экран не ждёт модель: он готов, как только есть вёрстка и шрифты.
   Индикатор загрузки — тонкая полоска сверху, она ничего не перекрывает. */
function pageReady() {
  document.body.classList.add('is-ready');
}
if (document.readyState === 'complete') pageReady();
else addEventListener('load', pageReady, { once: true });
document.fonts?.ready?.then(pageReady);
setTimeout(pageReady, 1500);              // страховка, если load почему-то не придёт

/* Модель доехала. Дорожку прокрутки под анимацию (640vh) разворачиваем только
   когда посетитель ещё на первом экране — иначе страница прыгнет у него под
   руками. Ушёл вниз — отдаём каталог обычной секцией и ждём возврата наверх. */
function enable3D() {
  if (document.body.classList.contains('is-3d') || staticMounted) return;
...
function finishLoading() {
  ...
  if (scrollY < innerHeight * 0.5) {
    requestAnimationFrame(enable3D);
    return;
  }
  /* посетитель уже листает — не дёргаем страницу */
  mountStaticCatalog();
  const back = () => {
    if (scrollY < innerHeight * 0.25) {
      removeEventListener('scroll', back);
      /* вернуть слой в сцену и включить анимацию */
      dom.stage.querySelector('.stage__sticky').appendChild(dom.layer);
      document.getElementById('catalog-static')?.remove();
      staticMounted = false;
      enable3D();
    }
  };
  addEventListener('scroll', back, { passive: true });
}
...
function update(force) {
  if (!document.body.classList.contains('is-3d')) return false;
  ...
  if (!M.ready) {
    if (true) {                         // модель ещё едет (или не приедет) — раскладка без неё
      const f = finalState();
...
document.querySelectorAll('a[href="#catalog"]').forEach(a => {
  a.addEventListener('click', e => {
    e.preventDefault();
    const st = document.getElementById('catalog-static');
    if (st || !document.body.classList.contains('is-3d')) {
      (st || dom.stage).scrollIntoView({ behavior: reduced ? 'auto' : 'smooth' });
      return;
    }
